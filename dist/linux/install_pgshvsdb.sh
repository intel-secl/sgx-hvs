...
ISECL_PGDB_DBNAME="${ISECL_PGDB_DBNAME:-pgshvsdb}"                # database name
ISECL_PGDB_USERNAME="${ISECL_PGDB_USERNAME:-dbshvsuser}"          # name of user used to connect to database
...
dnf -y install postgresql11-server postgresql11 postgresql11-contrib &>>$log_file

if [ $? -ne 0 ] ; then
	echo "dnf installation fail"
	exit 1
fi
...
sudo -E PGDATA=$PGDATA -E PGHOST=$PGHOST -E PGPORT=$PGPORT -u postgres psql postgres -c "CREATE EXTENSION IF NOT EXISTS \"uuid-ossp\" WITH SCHEMA public;" &>> $log_file
