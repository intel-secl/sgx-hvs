...
import (
	...
	consts "intel/isecl/sgx-host-verification-service/constants"
	"io"
	"io/ioutil"
	"net/http"
	"os"
...
func (jwt JWT) Run(c setup.Context) error {
	...
	var aasURL string
	if aasURL, err = c.GetenvString("AAS_BASE_URL", "AAS Server URL"); err != nil {
...
func fnGetJwtCerts(aasURL string) error {
	...
	log.Info("tasks/download_aas_jwtcert:fnGetJwtCerts() Connecting to AAS Endpoint %s", url)
